...
complete <- complete %>%
  mutate(weightgain = c(0, diff(weight)))
...
complete21 %>% 
  ggplot(mapping = aes(x=reorder(Diet, weightgain, FUN=median),
                                      y=weightgain)) + 
  geom_boxplot() + ylim(0, 30) + xlab("Diet") + ylab("Weight Gain") +
...
*Answer:* Some questions that come to mind after solving problem 8 is "Does the diet have anything to do with this
average? What is the growth rate for each diet?"
